import { r as registerInstance, c as createEvent, h, g as getElement } from './chunk-d9f44524.js';
...
var sum = adder();
var lengthSum = adder();
var areaSum$1 = adder(), areaRingSum$1 = adder();
...
var TreeClustering = /** @class */ (function () {
    ...
    function TreeClustering(sizeGenome, sequences, nbSec, minWord) {
        var _a = this.constructTree(sizeGenome, sequences, nbSec, minWord, 1, 0), children = _a[0], nivMax = _a[1];
        var totalWeight = 0;
        console.log('NIVEAU MAX : ' + nivMax);
        children.forEach(function (child) { return totalWeight += child.weight; });
        this.root = { 'min': 0, 'max': sizeGenome, 'children': children, 'weight': totalWeight };
        fname(this.root['children'], nivMax, nbSec);
    }
    TreeClustering.prototype.find = function (min, max, sequences) {
        // sequences in the sector
        var sequencesIn = {};
        var _loop_1 = function () {
            // coordinates of the sequence in the sector
            var tmpCoord = [];
            sequences[seq].forEach(function (coord) {
                // retrieve the start position of the sgRNA
                var start = /\(([0-9]*),/.exec(coord)[1];
    ...
    TreeClustering.prototype.constructTree = function (sizeSector, sequences, nbSec, minWord, niv, minPos) {
        // calcul interval
        var sizeSubSector = sizeSector / nbSec;
        var listChildren = [];
        var nivMax = niv;
        for (var i = 0; i < nbSec; i++) {
            // the last sector takes all last position
            var max_1 = (i != nbSec - 1) ? (i + 1) * sizeSubSector + minPos : sizeSector + 1 + minPos;
            var min_1 = i * sizeSubSector + minPos;
            // find sequences in the sector
            var sequencesLeaves = this.find(min_1, max_1, sequences);
            // if the number of sequences in the sector is < to the minWord, then children are sequences else
            // it's a node
            var _a = (nbWord(sequencesLeaves) < minWord) ? [sequencesLeaves, 1] : this.constructTree(max_1 - min_1, sequencesLeaves, nbSec, minWord, niv + 1, min_1), children = _a[0], nivDepth = _a[1];
            if (nivDepth > nivMax)
                nivMax = nivDepth;
            if (nbWord(sequencesLeaves) < minWord) {
                listChildren.push({ 'min': min_1, 'max': max_1, 'children': children, 'weight': nbWord(sequencesLeaves), 'niv': niv });
            }
            else {
                listChildren.push({ 'min': min_1, 'max': max_1, 'children': children, 'weight': nbWord(sequencesLeaves) });
            }
        }
        return [listChildren, nivMax];
    };
    return TreeClustering;
}());
var MyComponent = /** @class */ (function () {
    function MyComponent(hostRef) {
        ...
        this.changeOrgCard = createEvent(this, "changeOrgCard", 7);
        this.changeRefCard = createEvent(this, "changeRefCard", 7);
        this.changeSgrnaCard = createEvent(this, "changeSgrnaCard", 7);
        this.sgDataSection = createEvent(this, "sgDataSection", 7);
    }
    // *************************** CLICK ***************************
    MyComponent.prototype.handleChangeOrg = function (event) {
        this.orgSelected = event.detail;
        var all_data = JSON.parse(this.all_data);
        ...
        this.selectedSection = -1;
        new TreeClustering(this.sizeSelected, this.show_data, 4, 5);
        console.log("CLICK on " + this.orgSelected);
    };
    MyComponent.prototype.handleChangeOrgRefSgrna = function (event) {
    ...
    MyComponent.prototype.handleChangeRef = function (event) {
    ...
    MyComponent.prototype.handleChangeSgrna = function (event) {
        this.sgrnaSelected = event.detail;
    };
    MyComponent.prototype.emitOrgChange = function (event) {
        var val = event.currentTarget.innerText;
        this.changeOrgCard.emit(val);
    };
    MyComponent.prototype.emitRefChange = function (event) {
        var val = event.currentTarget.value;
        this.changeRefCard.emit(val);
    };
    MyComponent.prototype.emitSgrnaChange = function (event) {
        var val = event.currentTarget.value;
        this.changeSgrnaCard.emit(val);
    };
    MyComponent.prototype.emitsgData = function (event) {
        var msg = { allSgrna: JSON.stringify(event) };
        this.sgDataSection.emit(msg);
    };
    MyComponent.prototype.handleTest = function (event) {
        console.log("************************\n RECU\n*$$$$$$$$$$$");
    ...
    MyComponent.prototype.componentDidUpdate = function () {
        var _this = this;
        this.element.shadowRoot.querySelector('.genomeCircle').addEventListener("click", function () {
            _this.subSgrna = undefined;
            _this.selectedSection = -1;
            _this.sgrnaSelected = undefined;
    ...
    MyComponent.prototype.generateGenomicCard = function () {
        var width = this.diagonal_svg, height = this.diagonal_svg;
        ...
        var data = [];
        var dataOneSgrna = this.show_data[this.sgrnaSelected];
        ...
        var div = select(this.element.shadowRoot.querySelector(".genomeGraph"))
            .append('div')
        ...
        // Draw sgRNA
        select(this.element.shadowRoot.querySelector('svg'))
            .append('g')
            .selectAll('path')
            .data(data)
            .enter()
        ...
        function arcFunction(datum) {
            var end = +datum.sgRNA.length + +datum.start;
            datum.startAngle = 2 * Math.PI * datum.start * (1 / sizeGenome);
            var endAngle = 2 * Math.PI * end * (1 / sizeGenome);
    ...
    // *************************** SUNBURST **************************
    MyComponent.prototype.generatePlot = function () {
        var _this = this;
        var treeClustering = new TreeClustering(this.sizeSelected, this.show_data, 4, 7);
        var radius = this.diagonal_svg * 10 / 100 + this.diagonal_svg * 15 / 100, padInnerRadisu = this.diagonal_svg * 10 / 100 + this.diagonal_svg * 10 / 100;
        var root = partition().size([2 * Math.PI, radius])(hierarchy(treeClustering.root).sum(function (d) { return d['niv']; }));
        var maxChild = Math.max.apply(Math, treeClustering.root['children'].map(function (o) { return o.weight; }));
        var arc$1 = arc()
            .startAngle(function (d) { return d['x0']; })
            .endAngle(function (d) { return d['x1']; })
            .padAngle(function (d) { return Math.min((d['x1'] - d['x0']) / 2, 0.005); })
            .padRadius(radius / 2)
            .innerRadius(function (d) { return d['y0'] + padInnerRadisu; })
            .outerRadius(function (d) { return d['y1'] - 1 + padInnerRadisu; });
        var color = quantize()
            .domain([0, maxChild])
            // @ts-ignore
            .range(['#F7FACE', '#E0F6BF', '#C1F2B0', '#A3EDAA', '#96E7B9', '#8BE0CD', '#80CDD8', '#6DA7C3', '#5B81AD', '#4A5E95', '#3A3E7D']);
        var svg = select(this.element.shadowRoot.querySelector('#displayGenomicCard'));
        function findSgrnaChildren(list_children) {
            var allSgrna = [];
            for (var i in list_children) {
                // Il y a des enfants
                if (list_children[i].hasOwnProperty('children')) {
                    allSgrna = new Set(findSgrnaChildren(list_children[i].children).concat(allSgrna)).slice();
                    // Il n'y a plus d'enfants, mais il y a des données
                }
                else if (list_children[i].data.children != {}) {
                    allSgrna = new Set(Object.keys(list_children[i].data.children).concat(allSgrna)).slice();
                }
        ...
        svg.append('g')
            ...
            else if (i == _this.selectedSection) {
            ...
            _this.subSgrna = uniqSgrna;
            _this.sgrnaSelected = undefined;
            _this.selectedSection = i;
            var subData = {};
            Object.keys(_this.show_data).forEach(function (e) {
                if (_this.subSgrna.includes(e)) {
                    subData[e] = _this.show_data[e];
                }
            });
            _this.emitsgData(subData);
        });
        // Text
        svg.append("g")
            .attr('transform', 'translate(' + this.diagonal_svg / 2 + ', ' + this.diagonal_svg / 2 + ')')
            ...
            .selectAll("text")
            .data(root.descendants().filter(function (d) { return d.depth > 0 && (d.y0 + d.y1) / 2 * (d.x1 - d.x0) > 10; }))
            .enter().append("text")
            .attr("transform", function (d) {
            var x = (d.x0 + d.x1) / 2 * 180 / Math.PI;
            var y = (d.y0 + d.y1) / 2 + padInnerRadisu;
        ...
        var tempScale = linear$1()
            .domain([0, maxChild])
            .range([0, 11]);
        //Calculate the variables for the temp gradient
        var numStops = 10;
        var tempRange = tempScale.domain();
    ...
    // *************************** DISPLAY ***************************
    MyComponent.prototype.showCoord = function () {
        if (this.sgrnaSelected == undefined) {
            return "";
        }
        var dataOneSgrna = this.show_data[this.sgrnaSelected];
        var text = this.sgrnaSelected + " : " + dataOneSgrna.length + "\n";
        dataOneSgrna.forEach(function (coord) {
            text += coord + "\n";
        });
        return (text);
    };
    MyComponent.prototype.styleHelp = function (ref, target) {
    ...
    MyComponent.prototype.render = function () {
        ...
        var tabOrgName = this.org_names.split("&");
        var styleDisplay, all_data;
        ...
        var displayLoad = styleDisplay[0], displayGenomeCard = styleDisplay[1];
        if (this.all_data == undefined) {
            return ([
                h("div", { style: { display: displayLoad } }, h("strong", null, " Loading ... "), h("div", { class: "spinner-grow text-info", role: "status" }, h("span", { class: "sr-only" }, "Loading...")))
            ]);
        }
        else {
            return ([
                h("head", null, h("link", { rel: "stylesheet", href: "https://fonts.googleapis.com/icon?family=Material+Icons" })),
                /* ************************************************************* */
                /* ************* Main component with menu and card ************* */
                /* ************************************************************* */
                /* ************* Menu ************* */
                h("div", { class: "main-genome-card", style: { display: displayGenomeCard } }, h("ul", { class: "nav nav-tabs", id: "myTab", role: "tablist" }, tabOrgName.map(function (name) {
                    var classTag = "nav-link", bool = "false";
                    if (name == _this.orgSelected) {
                        classTag = "nav-link active";
                        bool = "true";
                    }
                    return h("li", { class: "nav-item" }, " ", h("a", { class: classTag, "data-toggle": "tab", role: "tab", "aria-selected": bool, href: "#", onClick: _this.emitOrgChange }, " ", name, " "), " ");
                })), h("div", { class: "tab-content genomeGraph", id: "myTabContent" }, h("div", { class: "test", style: { float: "left" } }, h("div", { class: "select-menu" }, h("span", null, "References"), h("select", { class: "custom-select", onChange: function (e) { return _this.emitRefChange(e); } }, this.genomeRef.map(function (ref) { return h("option", null, ref); }))), h("div", { class: "select-menu" }, h("span", null, "sgRNA"), h("select", { class: "custom-select", onChange: function (e) { return _this.emitSgrnaChange(e); }, style: { background: (this.subSgrna == undefined) ? "none" : "rgba(78, 195, 236, 0.2)" } }, h("option", null, "  "), (this.subSgrna == undefined) ?
                    (this.allSgrna.map(function (sgRna) { return (sgRna != _this.sgrnaSelected) ? h("option", null, sgRna) : h("option", { selected: true }, sgRna); })) :
                    (this.subSgrna.map(function (sgRna) { return (sgRna != _this.sgrnaSelected) ? h("option", null, sgRna) : h("option", { selected: true }, sgRna); }))))), h("div", null, h("p", { style: { padding: "12px 0px 0px 225px", margin: "Opx 0px 5px 0px" } }, " ", h("strong", null, " Coordinates Box ")), h("p", { class: "coordBox" }, this.showCoord())), h("div", { class: "help" }, h("i", { class: "material-icons" }, "help"), h("div", { class: "help-text help-gen" }, " Click on me to reinitialize sgRNA "), h("div", { class: "help-text help-section" }, " Click on me to display only sgRNA which are on me ")), h("svg", { id: 'displayGenomicCard', width: this.diagonal_svg, height: this.diagonal_svg }, this.generateGenomicCard(), h("text", { transform: "translate(" + (this.diagonal_svg / 2 - 30) + " , " + this.diagonal_svg / 2 + ")" }, " ", this.sizeSelected, " pb ")), this.generatePlot())),
                // @ts-ignore
    ...
    Object.defineProperty(MyComponent.prototype, "element", {
        get: function () { return getElement(this); },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(MyComponent, "style", {
        get: function () { return ".select-menu{width:200px;margin-top:15px;margin-bottom:15px;margin-left:5px}.select-menu span{margin-left:10px;font-weight:700}.tooltip-coord{position:absolute;text-align:left;width:220px;height:65px;padding:2px;font:12px sans-serif;background:#b0c4de;border:0;border-radius:8px;pointer-events:none}.spinner-grow{width:40!important;height:40!important}.axisCircles{fill:none;stroke:#e8e8e8;stroke-width:1px}.genomeCircle>path:hover,.sunburst>path:hover{cursor:pointer}.coordBox{margin-bottom:17px;margin-left:225px;padding-left:5px;height:105px;width:35%;overflow:scroll;white-space:pre-line;line-height:1.3em;border-style:inset;border-width:thin;border-radius:8px}.tooltip-coord>.material-icons{font-size:12px}.help:hover .help-text{display:block;position:absolute;background:rgba(103,203,222,.9);border-radius:8px;padding:8px}.help{width:24px}.help-text{display:none}.nav-tabs .nav-link.active{color:#4f5d75!important}.nav-link{color:#ef476f!important}.nav-link:hover{background:rgba(187,136,137,.3)!important;color:#4f5d75!important}.nav-tabs .nav-link.active:hover{background:none!important}.svg{position:relative}"; },
        enumerable: true,
        configurable: true
    });
    return MyComponent;
...
function DisplayGenome(root, width, height) {
    // Clean all arc
    select(root.querySelector('#displayGenomicCard')).selectAll('g').remove();
    var arcGenerator = arc();
    // Generator arc for the complete genome
    var pathGenome = arcGenerator({
        startAngle: 0,
        endAngle: 2 * Math.PI,
...
export { MyComponent as genomic_card };
