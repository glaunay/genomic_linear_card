import { r as registerInstance, h, g as getElement } from './chunk-64e0791b.js';
...
function formatLocale(locale) {
    var group = locale.grouping && locale.thousands ? formatGroup(locale.grouping, locale.thousands) : identity$1, currency = locale.currency, decimal = locale.decimal, numerals = locale.numerals ? formatNumerals(locale.numerals) : identity$1, percent = locale.percent || "%";
...
function transformer() {
    var domain = unit, range = unit, interpolate = interpolateValue, transform, untransform, unknown, clamp = identity$2, piecewise, output, input;
    function rescale() {
...
var MyComponent = /** @class */ (function () {
    ...
    // *************************** CLICK ***************************
    // *************************** FUNCTIONS ***************************
    MyComponent.prototype.initializeInterval = function () {
        var dicInterval = [];
        var stepInterval = Math.round((this.coordGene["end"] - this.coordGene["start"]) / this.nb_step);
        var start = this.coordGene["start"];
    ...
    MyComponent.prototype.checkOnGene = function (start, end) {
        if (start >= this.coordGene["start"] && end <= this.coordGene["end"]) {
            return true;
        }
        return false;
    };
    MyComponent.prototype.checkOnInterval = function (interval, start, end) {
        var stInt = interval.split("-")[0], endInt = interval.split("-")[1];
        if ((start >= stInt && start <= endInt) || (end >= stInt && end <= endInt) || (stInt >= start && stInt <= end)) {
            return true;
        }
        return false;
    };
    MyComponent.prototype.setDataHist = function () {
        var _this = this;
        var data = this.initializeInterval();
        Object.keys(this.allSgrna).forEach(function (sgrna) { return _this.allSgrna[sgrna].forEach(function (coord) {
            var start = coord.match('[+-][(]([0-9]*)')[1];
            var end = coord.match(',([0-9]*)[)]')[1];
            if (_this.checkOnGene(start, end)) {
                data.forEach(function (interval) {
                    if (_this.checkOnInterval(interval["stepCoord"], start, end)) {
                        interval["nbSgrna"] += 1;
    ...
    // *************************** DISPLAY ***************************
    MyComponent.prototype.displayHist = function () {
        var _this = this;
        var widthBarNb = this.width_bar.match("[0-9]*")[0];
        var leftBorder = (100 - widthBarNb) / 2;
        var color = "steelblue";
        var height = 200;
        selectAll(this.element.shadowRoot.querySelectorAll("#divHist>svg")).remove();
        selectAll(this.element.shadowRoot.querySelectorAll("#divHist>div")).remove();
        // Color scale for bin
        var colorScale = linear$1()
            .domain([min(this.dataHist, function (d) { return d["nbSgrna"]; }), max(this.dataHist, function (d) { return d["nbSgrna"]; })])
            // @ts-ignore
            .range([rgb(color).brighter(), rgb(color).darker()]);
        // set the ranges
        var x = band()
        ...
        var y = linear$1()
            .range([height, 0])
            .domain([0, max(this.dataHist, function (d) { return d["nbSgrna"]; })]);
        // Create the svg
        var svg = select(this.element.shadowRoot.querySelector("#divHist")).append("svg")
            .attr("width", this.width_bar)
            .attr("height", height)
            .style("margin-left", leftBorder + "%")
            .append("g");
        var div = select(this.element.shadowRoot.querySelector("#divHist"))
            .append('div')
        ...
        svg.selectAll(".bar")
            ...
            .on("click", function (e) {
            select(_this.element.shadowRoot.querySelector("#sgrnaBox")).html("<strong> Interval : </strong>" +
                e["stepCoord"] + "<strong> Nb sgRNA : </strong>" +
                e["nbSgrna"] + "<br/>" +
                e["sgrna"].map(function (sg) { return "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + sg + "<br/>"; }).join(' '));
        });
    };
    MyComponent.prototype.colorPagination = function (maxPages) {
        // Color arrows for pagination
        var colorBg = (this.page == 1) ? "#f1f1f1" : "rgba(239, 71, 111)";
        var colorArrow = (this.page == 1) ? "black" : "white";
    ...
    MyComponent.prototype.componentDidRender = function () {
        this.displayHist();
    ...
    MyComponent.prototype.render = function () {
        ...
        var widthBarNb = this.width_bar.match("[0-9]*")[0];
        var leftBorderGene = (100 - widthBarNb) / 2 + "%";
        var leftBorder = (100 - widthBarNb) / 2 - 2 + "%";
        var rightBorder = widthBarNb - ((100 - widthBarNb) / 2) + "%";
        var displayPagnigation = (this.pagniation) ? "block" : "none";
        return ([
            h("div", { id: "pagination", style: { display: displayPagnigation } }, h("a", { href: "#", class: "previous round", onClick: function () {
                    if (_this.page > 1) {
                        _this.page -= 1;
                        _this.coordGene = _this.allCoordGene[_this.page - 1];
                        _this.dataHist = _this.setDataHist();
                    }
                } }, "\u2039"), h("a", { href: "#", class: "next round", onClick: function () {
                    if (_this.page < _this.allCoordGene.length) {
                        _this.page += 1;
                        _this.coordGene = _this.allCoordGene[_this.page - 1];
                        _this.dataHist = _this.setDataHist();
                    }
                } }, "\u203A"), h("div", { class: "alert alert-primary alert-dismissible fade show", id: "alert-show", role: "alert" }, h("strong", null, "Holy guacamole!"), " You have ", this.allCoordGene.length, " homologous gene in this section.", h("button", { type: "button", class: "close", "data-dismiss": "alert", "aria-label": "Close" }, h("span", { "aria-hidden": "true", onClick: function () { return _this.element.shadowRoot.querySelector("#alert-show").id = "alert-hidden"; } }, "\u00D7")))),
            h("div", null, h("div", { id: "binSize" }, "Bin size : ", h("span", { style: { color: "rgb(239, 71, 111)", width: "35%" } }, Math.ceil((this.coordGene["end"] - this.coordGene["start"]) / this.nb_step))), h("div", { style: { float: "right", width: "35%" } }, h("strong", null, " Sgrna box "), h("div", { id: "sgrnaBox" }))),
            h("div", { id: "divHist" }, this.displayHist()),
            h("div", null, h("div", { class: "geneBar", style: { width: this.width_bar, marginLeft: leftBorderGene } }), h("span", { style: { marginLeft: leftBorder } }, " ", this.coordGene["start"], " "), h("span", { style: { marginLeft: rightBorder } }, " ", this.coordGene["end"], " ")),
            // @ts-ignore
    ...
    Object.defineProperty(MyComponent, "style", {
        get: function () { return ".geneBar{background:#ef476f;height:30px}.tooltip-hist{text-align:left;padding:2px;font:12px sans-serif;background:#b0c4de;border:0;border-radius:8px;pointer-events:none}#binSize{font-size:1.2em;float:left;margin-left:5%}#sgrnaBox{padding-left:5px;height:105px;width:90%;overflow:scroll;line-height:1.3em;border-style:inset;border-width:thin;border-radius:8px}rect:hover{cursor:pointer}.previous{background-color:#f1f1f1;color:#000}.next,.previous{text-decoration:none;display:inline-block;padding:8px 16px}.next{background-color:#ef476f;color:#fff}.round{border-radius:50%}.alert{display:inline-grid;margin-left:20px}#alert-hidden{-webkit-transform:translate(5000px);transform:translate(5000px);-webkit-transition:-webkit-transform 5s;transition:-webkit-transform 5s;transition:transform 5s;transition:transform 5s,-webkit-transform 5s}"; },
        enumerable: true,
        configurable: true
    });
    return MyComponent;
}());
export { MyComponent as linear_card };
